...
static THREAD_FUNC_PREFIX hdhomerun_video_thread_execute(void *arg)
{
	...
	while (!vs->terminate) {
		...
		/* Receive. */
		int length = recv(vs->sock, (char *)ptr, VIDEO_DATA_PACKET_SIZE, 0);
...
{
	hdhomerun_debug_printf(dbg, "%s video sock: pkt=%ld te=%ld miss=%ld drop=%ld\n", prefix, vs->packet_count, vs->transport_error_count, vs->sequence_error_count, vs->overflow_error_count);
}
