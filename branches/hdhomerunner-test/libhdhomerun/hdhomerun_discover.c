...
static bool_t hdhomerun_discover_send_internal(struct hdhomerun_discover_sock_t *dss, uint32_t target_ip, uint32_t device_type, uint32_t device_id)
{
	if (target_ip == 0) {
	...
	if (sendto(dss->sock, (char *)buffer, (int)length, 0, (struct sockaddr *)&sock_addr, sizeof(sock_addr)) != length) {
		return FALSE;
	}
...
static bool_t hdhomerun_discover_send(struct hdhomerun_discover_t *ds, uint32_t target_ip, uint32_t device_type, uint32_t device_id)
{
	...
	for (i = 0; i < ds->sock_count; i++) {
		struct hdhomerun_discover_sock_t *dss = &ds->socks[i];
		result |= hdhomerun_discover_send_internal(dss, target_ip, device_type, device_id);
	}

	return result;
}

static int hdhomerun_discover_recv_internal(struct hdhomerun_discover_sock_t *dss, struct hdhomerun_discover_device_t *result)
{
	uint8_t buffer[1024];
	struct sockaddr_in sock_addr;
	memset(&sock_addr, 0, sizeof(sock_addr));
	socklen_t sockaddr_size = sizeof(sock_addr);
	int rx_length = recvfrom(dss->sock, (char *)buffer, sizeof(buffer), 0, (struct sockaddr *)&sock_addr, &sockaddr_size);
	if (rx_length <= 0) {
...
static int hdhomerun_discover_recv(struct hdhomerun_discover_t *ds, struct hdhomerun_discover_device_t *result)
{
	...
	for (i = 0; i < ds->sock_count; i++) {
		...
		if (hdhomerun_discover_recv_internal(dss, result) <= 0) {
			continue;
		}
